...
pub fn future_schema_user_message(error: &str) -> String {
    format!(
        "当前数据已由更高版本的 PiHub 打开或迁移，不能回退到旧版本继续启动。\n\n请升级到最新版本后再打开应用，或恢复使用旧版本创建的兼容备份。\n\n技术信息：{error}"
    )
}

pub fn future_backup_schema_user_message(error: &str) -> String {
    format!(
        "这个备份由更高版本的 PiHub 创建，当前版本无法恢复。\n\n请升级 PiHub 后再恢复，或选择旧版本创建的兼容备份。\n\n技术信息：{error}"
    )
}
...
fn migrate_v8(_conn: &Connection) -> Result<(), String> {
    Ok(())
}
...
fn create_initial_indexes(conn: &Connection) -> Result<(), String> {
    for table in [DbTable::PiPromptConfig] {
    ...
    for table in [DbTable::FavoriteMcp] {
