...
- Skills 的唯一源目录始终是中央仓库 `central_repo_path`。Pi 或任何自定义工具当前运行时的 skills 目录都只是目标目录，不是同步源。
...
Skills 模块提供 AI 编程工具技能的统一管理功能。用户可以从本地文件夹或 Git 仓库安装技能，并同步到 Pi（以及用户自定义工具）。模块还支持技能发现、导入现有技能、系统托盘快捷菜单等功能。
...
| user_note | string? | PiHub 内部自定义备注，不写入 SKILL.md，不参与内容哈希 |
| management_enabled | bool | PiHub 管理启用状态；不是 `status` 健康状态，false 表示 UX 禁用并从当前工具取消同步 |
| disabled_previous_tools | array | 禁用前记录的工具绑定 key；重新启用时用于默认恢复勾选 |
| enabled_tools | array | 已启用的工具列表，如 ["pi"] |
...
5. **扫描工具 skills 目录**
   - 读取相对工具的 relative_skills_dir 下的所有子目录
   - 检测是否为符号链接/接合点，记录 link_target
   - 额外第三方目录（EXTRA_SKILL_SOURCES）已随 CC Switch 移除，当前不扫描第三方来源
...
- Pi（及用户自定义工具）的运行时 skills 目录只是同步目标，不是源。
...
**和配置路径联动的特殊点：**
- `runtime_location::get_tool_skills_path_async` 会根据模块当前配置目录，动态解析 Pi（及自定义工具）的 skills 目标目录。
...
`management_enabled` 是 PiHub 内部管理状态，不能复用 `status`。`status` 仍表示 skill 内容或同步健康状态（如 ok/error）。
...
Inventory JSON 是完整管理清单，用于重排 PiHub 元数据，不是 skill 内容备份，也不是局部 patch。
...
当前内置支持的 Skills 工具（PiHub 仅保留 Pi，其余已移除）：
...
1. 尝试 symlink（Unix 或 Windows 管理员权限）
2. Windows 回退到 junction（目录接合点，无需管理员）
4. 最终回退到 copy（完整复制目录）
...
- 默认路径：`~/.agents/skills`（home 目录下的 .agents/skills）
...
- 用户备注、手动分组、管理启用状态都属于 PiHub 管理元数据，不属于中央仓库文件内容。分组事实源分拆为 `skill_group` + `skill.group_id`，`user_group` 仅为兼容名称；更新、同步、WSL/SSH 后续链路不能因为这些元数据变化而改写 `SKILL.md` 或发起技能内容同步。
...
- 后端计算 `newly_installed`，可提示用户同步现有技能到新工具（当前前端无水到的新工具 UI 入口；若未来需要，应新增触发入口，而不是复用已删除的 NewToolsModal）
...
- WSL 自动同步
  ...
  - Pi 本机自定义根目录只改变本机运行时 skills 目标，例如 `<custom-root>/skills`；普通 WSL 侧工具目录仍按 Pi 默认 `~/.pi/agent/skills` 维护。只有 Pi 当前运行时本身是 WSL Direct 自定义根目录时，目标才跟随该 Linux 根目录。
...
### 8.8 Pi 在 WSL 自定义配置下对 Skills 的影响

- 这里的工具是 Pi（以及用户自定义工具）。
- `runtime_location` 会先解析 Pi 当前的**生效配置路径**，然后再判断它是不是 WSL UNC 路径。
- 一旦 Pi 被判定为 `is_wsl_direct=true`：
  - Pi 的运行时 skills 目录也会跟着切到对应的 WSL UNC 路径。
  - `skills_sync_to_tool` 仍然从中央仓库取源，只是同步目标改成了这份 WSL UNC 路径。
  - WSL 自动同步在给 WSL 统一中央仓库建工具链接时，应跳过 Pi，避免对已经直接运行在 WSL 内的工具目录重复维护。

- Pi 以根目录为核心，从根目录派生配置文件、prompt、skills 等路径。

- 所以排查 "为什么 Skills 目标目录变成 WSL" 时，不要只看 Skills 模块本身，要回头确认 Pi tab 的配置路径是否已经被 `runtime_location` 判定为 WSL direct。
