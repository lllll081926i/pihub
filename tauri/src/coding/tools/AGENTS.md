...
- 内置工具定义来自 `builtin.rs` 的静态配置。当前仅保留 `pi`（PiHub 只管理 Pi 一个 CLI）；其他第三方 CLI 不作为内置工具，用户可通过自定义工具（`custom_tool` 表）扩展。
- 用户自定义工具来自主数据库的 `custom_tool` 表；必须直接读写 SQLite JSONB，旧 SurrealDB 仅用于启动时一次性导入。这部分是 Skills/MCP 对"额外工具"的唯一持久化来源。
- 对 runtime root 可配置的内置工具（当前为 `pi`），真正的 MCP/Skills 路径不能只看静态字符串，必须优先经过 `runtime_location` 派生。

## 核心设计决策（Why）

- Skills 和 MCP 共用同一套 `RuntimeTool` 抽象，避免两个功能各维护一套工具列表和检测规则。
- 自定义工具字段分为 Skills 相关和 MCP 相关，保存时要保留另一侧字段，不能互相覆盖。
- 对 runtime root 可配置的内置工具（当前为 `pi`），带数据库上下文的路径解析必须优先于静态默认值，否则 WSL Direct 场景会错。
...
- 内置工具的 Skills/MCP 路径在 WSL Direct 场景下必须用 `*_with_db` 版本解析，不能退回静态默认路径。
...
- 至少验证：WSL Direct 场景下内置工具的 MCP/Skills 路径仍从 runtime_location 解析。