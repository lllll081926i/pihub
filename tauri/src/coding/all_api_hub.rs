...
/// List All API Hub providers with resolved API keys.
#[tauri::command]
pub async fn list_all_api_hub_providers(
    db_state: tauri::State<'_, SqliteDbState>,
) -> Result<AllApiHubDiscovery, String> {
    list_provider_candidates_with_keys(&db_state).await
}

/// Resolve a specific set of All API Hub providers with API keys.
#[tauri::command]
pub async fn resolve_all_api_hub_providers(
    db_state: tauri::State<'_, SqliteDbState>,
    provider_ids: Vec<String>,
) -> Result<Vec<AllApiHubProviderCandidate>, String> {
    resolve_provider_candidates_with_keys(&db_state, &provider_ids).await
}
...
fn parse_providers_from_storage(
    ...
    for account in raw_accounts {
        ...
        let api_protocol = infer_api_protocol(&npm);
...
fn infer_api_protocol(npm: &str) -> String {
    match npm {
