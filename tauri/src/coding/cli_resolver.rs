...
pub fn local_cli_missing_hint(command_name: &str) -> String {
    format!(
        "未找到 `{command_name}` CLI。PiHub 已检查当前 PATH、常见安装路径，以及 nvm、volta、fnm、nvm-windows、bun、mise、asdf 管理的全局 bin；macOS 从 Dock/Finder/Spotlight 启动时不会继承终端 shell PATH。请确认 CLI 已安装。"
    )
}
...
mod tests {
    ...
    fn fnm_default_alias_candidate_precedes_scanned_versions() {
        ...
        let candidates = collect_fnm_candidates(&fnm_dir, "pi");

        #[cfg(target_os = "windows")]
        {
            assert_eq!(candidates.first(), Some(&default_alias.join("pi")));
            assert!(candidates.contains(&default_alias.join("pi.cmd")));
            assert!(candidates.contains(&version_installation.join("pi")));
        }

        #[cfg(not(target_os = "windows"))]
        assert_eq!(candidates.first(), Some(&default_bin.join("pi")));

        assert!(candidates.contains(&version_bin.join("pi")));
    }
    ...
    fn select_windows_command_path_prefers_cmd_over_extensionless() {
        let selected = super::select_command_path(&[
            PathBuf::from(r"C:\Users\tester\AppData\Roaming\fnm\aliases\default\pi"),
            PathBuf::from(r"C:\Users\tester\AppData\Roaming\fnm\aliases\default\pi.cmd"),
            PathBuf::from(r"C:\Users\tester\AppData\Roaming\fnm\aliases\default\pi.ps1"),
        ])
