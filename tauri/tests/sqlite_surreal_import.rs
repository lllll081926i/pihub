...
async fn imports_selected_surreal_tables_into_sqlite_jsonb() {
    ...
    surreal
        .query("UPSERT pi_prompt_config:`provider-a` CONTENT $data")
        .bind((
            "data",
            json!({
                "name": "Prompt A",
    ...
    surreal
        .query("UPSERT pi_prompt_config:`provider-b` CONTENT $data")
        .bind((
            "data",
            json!({
                "name": "Provider B",
    ...
    let report = import_tables_from_surreal(
        &sqlite_state,
        &surreal,
        &[DbTable::Settings, DbTable::PiPromptConfig],
    )
    .await
    ...
    let providers = sqlite_state
        .with_conn(|conn| db_list(conn, DbTable::PiPromptConfig, Some(&order)))
...
async fn missing_known_table_import_preserves_non_empty_sqlite_tables() {
    ...
    surreal
        .query("UPSERT pi_prompt_config:`surreal-prompt` CONTENT { name: 'Surreal Prompt', content: 'hello' }")
        .await
    ...
    assert!(report.tables.iter().any(|table| {
        table.table == DbTable::PiPromptConfig.name() && table.surreal_count == 1
    }));
    assert!(!report
        .tables
        .iter()
    ...
    let provider = sqlite_state
        .with_conn(|conn| db_get(conn, DbTable::PiPromptConfig, "surreal-prompt"))
