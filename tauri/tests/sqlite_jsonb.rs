...
fn jsonb_probe_and_schema_migration_create_all_tables() {
    ...
    for index_name in [
        "idx_pi_prompt_config_is_applied",
        "idx_pi_prompt_config_sort_index",
        "idx_favorite_mcp_created_at",
        "idx_skill_name",
        "idx_mcp_server_name",
        "idx_favorite_mcp_name",
    ] {
        let exists: i64 = conn
            .query_row(
...
fn future_backup_schema_message_is_user_facing_and_detectable() {
    ...
    assert!(message.contains("这个备份由更高版本的 PiHub 创建"));
...
fn put_derives_columns_from_numeric_json_timestamps_without_changing_json_type() {
    ...
    .expect("put prompt config");

    let (column_created_at, json_created_type): (String, String) = conn
        .query_row(
            "SELECT created_at, json_type(data, '$.created_at')
             FROM pi_prompt_config WHERE id = 'prompt-1'",
            [],
            |row| Ok((row.get(0)?, row.get(1)?)),
        )
        .expect("read numeric timestamp");
    let record = db_get(&conn, DbTable::PiPromptConfig, "prompt-1")
        .expect("get prompt config")
        .expect("prompt config record");
...
fn list_orders_by_json_integer_then_column() {
    ...
    let records = db_list(&conn, DbTable::PiPromptConfig, Some(&order)).expect("list");
