...
需求来源：GitHub issue `https://github.com/lllll081926i/pihub/issues/227`。

用户希望 PiHub 支持把已有的全局 Skills 目录作为统一源目录，例如：
...
当前 PiHub 已经有 Skills 中央仓库概念，后端也已经通过 `skill_settings:skills.central_repo_path` 保存中央仓库路径。但当前前端设置页只是展示路径，没有提供完整的更改、预览、扫描和纳管流程；现有导入逻辑也主要是把本地或 Git 来源复制到中央仓库，而不是把一个已有全局目录直接作为长期源目录接入。
...
2. PiHub 在保存前展示路径切换预览，包括新目录里可识别的 Skill、已有管理记录的匹配情况、缺失项和冲突项。
3. 如果用户是从旧中央仓库目录迁移到新目录，PiHub 提供迁移预览，并允许把旧目录中已管理的 Skill 复制到新目录。
4. 用户确认后，PiHub 将该目录保存为中央仓库路径。
...
用户在外部编辑器修改全局目录后，PiHub 不需要实时刷新 hash。可以在以下动作中刷新：
...
原因是用户看到的是自己选择的全局目录，不会自然区分“这个目录里的某个 Skill 是 PiHub 从 Git 复制来的副本”还是“用户自己维护的全局目录源”。如果要删除源文件，必须走同一个危险选项：
...
`恢复默认` 不应藏在更多菜单里。它是路径设置的直接反向操作，应与 `更改` 放在同一行，便于用户从自定义全局目录退回 PiHub 默认目录。
...
```text
纳管只会创建 PiHub 管理记录，不会移动、复制或覆盖源目录文件。
```
...
```text
将取消 PiHub 对「code-review」的管理，并从已同步工具中清理目标目录。源目录不会被删除：
...
- `远端同步镜像目录`：PiHub 为 WSL/SSH 同步维护的远端 staging/mirror 目录，当前实现是 `~/.ai-toolbox/skills`。
...
默认 app data 中央仓库属于 PiHub 管理目录。如果现有备份策略已经备份默认 Skills 目录内容，恢复后应能把默认中央仓库内容恢复到新 app data 目录，并让 managed skill 记录正常解析。
...
原因是这些目录可能很大，也可能包含用户并不希望放进 PiHub 备份包的 Git 历史、私有文件或外部工具数据。第一版应只备份“指向它的配置和管理记录”，不默认备份外部目录内容。
...
- `恢复默认`：切回 PiHub 默认 app data 目录，但仍走 preview/apply。
...
16. 本机全局目录改成 `~/.agent/skills` 后，WSL/SSH 远端镜像目录是否也改成 `~/.agent/skills`？
   - 建议：第一版不改。远端仍使用 PiHub 管理的 `~/.ai-toolbox/skills` 作为镜像目录；远端自定义镜像目录作为第二期 per-distro / per-connection 高级设置单独设计。
