...
shadows:
  ...
  lg: "0 8px 28px rgba(0,0,0,0.12), 0 4px 12px rgba(0,0,0,0.06)"
motion:
...
`DESIGN.md` 是给 AI coding agents 和开发者阅读的视觉设计系统文档。它描述 PiHub 的产品调性、设计 token、布局密度、组件形态和常见 Do / Don't。

本文件不是应用运行时资源。除非某个任务明确要求建立运行时设计 token 管线，否则不要把它接入构建、主题切换、备份、同步或用户配置读写流程。

## 一句话调性

PiHub 是面向长期使用的桌面端配置工作台。界面应当安静、克制、高信息密度、可靠、易扫描，服务于反复管理 Coding CLI、模型供应商、Gateway、会话、Skills、MCP、WSL/SSH、备份和图片渠道。
