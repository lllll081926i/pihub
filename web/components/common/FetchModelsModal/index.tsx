...
const FetchModelsModal: React.FC<FetchModelsModalProps> = ({
  ...
  const handleFetch = async () => {
    ...
    try {
      ...
      if (response.models.length === 0) {
        message.info(t('piModels.fetchModels.noModelsFound'));
      }
    } catch (err) {
      const errorMsg = err instanceof Error ? err.message : String(err);
      setError(errorMsg);
      message.error(t('piModels.fetchModels.fetchFailed'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const columns = [
    {
      title: t('piModels.fetchModels.modelId'),
      dataIndex: 'id',
      key: 'id',
      render: (id: string) => {
          ...
          <div className={styles.modelIdCell}>
            ...
            {isExisting && (
              <Tag bordered={false} className={styles.existingTag}>
                {t('piModels.fetchModels.alreadyExists')}
              </Tag>
            )}
          </div>
        );
    ...
    {
      title: t('piModels.fetchModels.ownedBy'),
      dataIndex: 'ownedBy',
      key: 'ownedBy',
      width: 150,
  ...
  const summaryItems = [
    {
      label: t('piModels.fetchModels.returnedCount'),
      value: models.length,
    },
    {
      label: t('piModels.fetchModels.selectedCount'),
      value: selectedRowKeys.length,
    },
    {
      label: t('piModels.fetchModels.removableCount'),
    ...
    <Modal
      ...
      title={
        <Space>
          ...
          {t('piModels.fetchModels.title', { provider: providerName })}
        </Space>
      }
      open={open}
      onCancel={onCancel}
      ...
      footer={[
        ...
        >
          {t('piModels.fetchModels.applyChanges', {
      ...
      <div className={styles.content}>
        <section className={styles.sectionCard}>
          <div className={styles.sectionHeader}>
            <div className={styles.sectionTitle}>{t('piModels.fetchModels.sourceSection')}</div>
            <Text className={styles.sectionHint}>{t('piModels.fetchModels.sourceSectionHint')}</Text>
          </div>

          <div className={`${styles.fieldBlock} ${styles.fieldRow}`}>
            <Text strong className={styles.fieldLabel}>
              {t('piModels.fetchModels.apiType')}
            </Text>
            <div>
              <div className={styles.apiTypePanel}>
                ...
                >
                  <Radio value="openai_compat">
                    {t('piModels.fetchModels.openaiCompat')}
                  ...
                  {supportsNative && (
                    <Radio value="native">
                      {t('piModels.fetchModels.native')}
                      <Text type="secondary" style={{ marginLeft: 8, fontSize: 12 }}>
                        ({t('piModels.fetchModels.nativeHint')})
                      </Text>
                    </Radio>
                  )}
                </Radio.Group>
        ...
        <section className={styles.sectionCard}>
          <div className={styles.sectionHeader}>
            <div className={styles.sectionTitle}>{t('piModels.fetchModels.resultSection')}</div>
            <Text className={styles.sectionHint}>{t('piModels.fetchModels.resultSectionHint')}</Text>
          </div>
          ...
          <div className={styles.toolbar}>
            <Button
              ...
              onClick={handleFetch}
            >
              {fetched ? t('piModels.fetchModels.refresh') : t('piModels.fetchModels.fetch')}
            </Button>
            <Input
              className={styles.searchInput}
              prefix={<SearchOutlined />}
              placeholder={t('piModels.fetchModels.searchPlaceholder')}
              value={searchText}
          ...
          {error && (
            <Alert
              ...
              type="error"
              message={t('piModels.fetchModels.fetchFailed')}
              description={error}
              showIcon
              closable
          ...
          {fetched && missingModelCount > 0 && (
            <div className={styles.cleanupCard}>
              ...
              >
                {t('piModels.fetchModels.removeMissing', { count: missingModelCount })}
              </Checkbox>
            </div>
          )}
          ...
          {fetched && missingModelCount === 0 && (
            <div className={styles.cleanupMuted}>
              <Text type="secondary">
                {t('piModels.fetchModels.removeMissingNone')}
              </Text>
            </div>
          )}
          ...
          {fetched && (
            <div className={styles.tableWrap}>
              <Table
                ...
                size="small"
                locale={{
                  emptyText: searchText ? t('piModels.fetchModels.noSearchResults') : t('piModels.fetchModels.noModelsFound'),
                }}
              />
            </div>
          )}
