...
- `user_group/user_note` 是 PiHub 内部的用户管理元数据，事实源是后端 `mcp_server` 记录，不是 MCP server 自身配置或工具运行时配置文件。
...
- 不要把 MCP 自身的 `description` 和 PiHub 管理备注 `user_note` 合并存储；卡片展示可以在 `user_note` 为空时回退展示 `description`，但编辑入口必须分开。
...
- PiHub 仅内置 Pi 一个工具，MCP 服务器默认同步目标就是 Pi（新建/导入默认启用）。
- Pi 的 MCP 通过 pi-mcp-adapter 扩展消费 `~/.pi/agent/mcp.json` 的标准 `mcpServers` 配置。MCP 管理页加载时检测 adapter 是否安装（`check_mcp_adapter_installed`：检查 Pi runtime `npm/node_modules/pi-mcp-adapter/package.json`）；未安装时提示并禁用添加/导入/配置，`install_mcp_adapter` 执行 `pi install npm:pi-mcp-adapter --no-approve`，安装完成后重新检测并开放。
...
- 超时设置在同一个表单区块里合并展示，但语义不能混：顶层 `timeout`（毫秒）与 `server_config.startup_timeout_sec` / `tool_timeout_sec`（秒）分别落盘到不同字段；留空都不落盘。秒级字段仅用于从其他 CLI 配置导入时的兼容读取（导入来源扫描保留），Pi 运行时消费的是顶层 `timeout` 毫秒值。
