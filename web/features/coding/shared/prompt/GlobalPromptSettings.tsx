...
interface GlobalPromptSettingsProps {
  ...
  /**
   * Render as a flat section (header + content, no Collapse wrapper).
   * Defaults to the legacy collapsible panel behavior.
   */
  variant?: 'collapse' | 'flat';
}
...
const GlobalPromptSettings: React.FC<GlobalPromptSettingsProps> = ({
  ...
  const sectionTitle = (
    <div className={styles.flatSectionTitle}>
      <span className={styles.flatSectionTitleText}>
      ...
      <div className={styles.flatSectionTitleActions}>
        <Button
          type="link"
          size="small"
          style={{ fontSize: 12 }}
