...
>({
  ...
  const filteredProviders = React.useMemo(() => result?.providers || [], [result]);
  ...
  const loadProviders = React.useCallback(async () => {
    ...
    try {
      ...
      if (translatedMessage && data.providers.length === 0) {
  ...
  }, [listProviders, t, translateAllApiHubMessage]);
  ...
  const handleResolveToken = React.useCallback(async (providerId: string) => {
    ...
    const matched = resolved.find((provider) => getProviderId(provider) === providerId);
  ...
  }, [getProviderId, mapProviderToItem, resolveProviders]);
