...
interface SessionManagerPanelProps {
  ...
  /** 扁平模式：移除 Collapse 包装，直接显示内容（用于嵌入页面） */
  flat?: boolean;
}
...
const SessionManagerContent: React.FC<SessionManagerContentProps> = ({
  ...
  // 分页：full load 后 items 为全量，前端切片分页，避免长列表撑高页面
  ...
  React.useEffect(() => {
    setCurrentPage(1);
  }, [debouncedQuery, pathFilter, sourceMode, tool]);

  // items 缩减（删除/刷新）时钳制页码，避免停留在不存在的页
  React.useEffect(() => {
  ...
  const handleSelectAllCurrentPage = () => {
    const currentPagePaths = pageItems.map((session) => session.sourcePath);
...
const SessionManagerPanel: React.FC<SessionManagerPanelProps> = ({
  ...
  const { t } = useTranslation();
  const [expanded, setExpanded] = React.useState(flat);
