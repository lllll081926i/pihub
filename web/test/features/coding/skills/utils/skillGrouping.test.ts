...
test('skill group tool helpers use union and detect mixed tool sets', () => {
  const group = {
    ...
    skills: [
      makeSkill({
        id: 'a',
        targets: [
          { tool: 'pi', mode: 'link', status: 'ok', target_path: '', synced_at: 1 },
          { tool: 'tool', mode: 'link', status: 'ok', target_path: '', synced_at: 1 },
        ],
      }),
      makeSkill({
        id: 'b',
  ...
  assert.deepEqual(getSkillGroupToolIds(group).sort(), ['pi', 'tool']);
  assert.equal(isSkillGroupToolsAligned(group), false);
  assert.deepEqual(getSkillIdsMissingTool(group, 'tool'), ['b']);
  assert.deepEqual(getSkillIdsWithTool(group, 'pi'), ['a', 'b']);
});
...
test('skill group tool helpers ignore disabled skills for group tool mode', () => {
  ...
  assert.deepEqual(getSkillGroupToolIds(group), ['pi']);
  assert.equal(isSkillGroupToolsAligned(group), false);
  assert.deepEqual(getSkillIdsMissingTool(group, 'pi'), ['enabled-b']);
  assert.deepEqual(getSkillIdsWithTool(group, 'pi'), ['enabled-a']);
});
