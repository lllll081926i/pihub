...
test('mcp group tool helpers use union and detect mixed tool sets', () => {
  const group = {
    ...
    label: 'Dev',
    servers: [
      makeServer({ id: 'a', enabled_tools: ['pi', 'tool'] }),
      makeServer({ id: 'b', enabled_tools: ['pi'] }),
    ],
  };

  assert.deepEqual(getMcpGroupToolKeys(group).sort(), ['pi', 'tool']);
  assert.equal(isMcpGroupToolsAligned(group), false);
  assert.deepEqual(getMcpServerIdsMissingTool(group, 'tool'), ['b']);
  assert.deepEqual(getMcpServerIdsWithTool(group, 'pi'), ['a', 'b']);
});
...
test('mcp group tool helpers treat equal sets in different order as aligned', () => {
  const group = {
    ...
    label: 'Dev',
    servers: [
      makeServer({ id: 'a', enabled_tools: ['tool', 'pi'] }),
      makeServer({ id: 'b', enabled_tools: ['pi', 'tool'] }),
    ],
  };
