...
export function Wrappers({ t, code, i18nPrefix }: any) {
  ...
  if (code === 'GIT_NOT_FOUND') {
    confirmMessageKey = 'piModels.provider.baseUrlConfirmV1';
  } else {
    confirmMessageKey = 'piModels.provider.baseUrlConfirmV1Beta';
  }
  return [
...
`,
  ...
  assert.ok(usedKeys.has('piModels.provider.baseUrlConfirmV1'));
  assert.ok(usedKeys.has('piModels.provider.baseUrlConfirmV1Beta'));
  ...
  assert.equal(analysis.unusedLocaleKeys.some((entry) => entry.key === 'piModels.provider.baseUrlConfirmV1'), false);
  assert.equal(analysis.unusedLocaleKeys.some((entry) => entry.key === 'piModels.provider.baseUrlConfirmV1Beta'), false);
});
